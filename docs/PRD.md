...
- **Solution:** Use the `python-magic` library (libmagic) combined with header sniffing and `mimetypes` as a fallback to guarantee accurate file type detection globally. Additionally, specific noise extensions (such as `.xml`, `.css`, and `.js`) and noise MIME types (such as `application/xml`) are explicitly ignored during directory ingestion to prevent clutter and database bloat.
