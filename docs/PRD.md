...
- **Solution:** Implement a dynamic plugin loader.
  ...
  - Developers simply create a new file in `src/plugins/` and decorate their class with `@register_analyzer(name=ESTATE_ANALYZER_NAME, depends_on=[TEXT_EXTRACTOR_NAME])`. Current plugins include `TextExtractorPlugin`, `RouterPlugin`, `MetadataExtractorPlugin`, `VisionAnalyzerPlugin`, `DeepSummarizerPlugin`, `PasswordExtractorPlugin`, `SpreadsheetAnalyzerPlugin`, `PIIHarvesterPlugin`, `DuplicateDetectorPlugin`, `OCRConfidenceScorerPlugin`, `VideoAnalyzerPlugin`, `DocumentAIExtractorPlugin`, `EstateAnalyzerPlugin`, `AudioTranscriberPlugin`, `SummarizerPlugin`, `LanguageDetectorPlugin`.
