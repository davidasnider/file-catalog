...
- **Schema Design:**
  - `Document`: Tracks path, robustly detected MIME type, file hash (to prevent duplicate processing if moved), and overall status (`PENDING`, `EXTRACTING`, `ANALYZING`, `COMPLETED`, `FAILED`, `NOT_PRESENT`). Note that `NOT_PRESENT` is a core status used for filesystem synchronization to mark files that have been deleted or moved.
