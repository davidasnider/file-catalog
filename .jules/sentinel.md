...
**Learning:** `py7zr` `extractall` does not have a safe filter option like `tarfile` does. We need to manually extract only verified members. `py7zr` uses `archive.list()` not `archive.get_files()`.
**Prevention:** Iterating over `archive.list()` and extracting specific targets using `archive.extract(..., targets=[...])` is the safer approach for 7z files in `py7zr`.
