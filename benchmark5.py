...
async def block_loop():
    # this will simulate a blocking operation
    time.sleep(1)
...
async def main():
    ...
    # 1. Benchmark block_loop directly so we see it register
    print("--- Testing blocking measurement ---")
    delays = []
    ...
    delays = []
    monitor_task = asyncio.create_task(monitor_event_loop(delays))
    await asyncio.sleep(0.05) # give monitor task chance to run
