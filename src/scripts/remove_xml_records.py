...
async def cleanup():
    ...
    async with async_session_maker() as session:
        # Query for documents that are either .xml files or have XML MIME types
        statement = select(Document).where(
            (Document.path.like("%.xml"))
            | (
        ...
        # 1. Remove from Full-Text Search (FTS)
        try:
            await remove_documents_from_fts(session, doc_ids)
            logger.info("Successfully removed documents from FTS index.")
        except Exception as e:
            logger.warning(f"Failed to remove some documents from FTS: {e}")

        # 2. Delete AnalysisTasks associated with these documents
        # We delete in batches to avoid SQLite parameter limits if there are many docs
        batch_size = 500
