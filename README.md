...
- **Streamlit Dashboard**: A beautiful dashboard at `localhost:8501` featuring:
  ...
  - **Full-Text Search**: Search through extracted text, AI summaries, and Vision descriptions using SQLite FTS5. To securely render search snippets without allowing unsafe HTML, the UI replaces custom delimiters (`[HL_START]` and `[HL_END]`) generated by the FTS query with Markdown bold formatting. Search queries are also sanitized to prevent SQL injection vulnerabilities.
...
- **UI Query Optimization**: The Streamlit dashboard uses SQLite's `json_each()` function in helper functions to fetch and expand task lists for multiple documents in a single query, bypassing standard SQL parameter limits with a chunked fallback for non-SQLite backends.
...
- **Quick Skip Mechanism:** The system tracks `file_size` and `mtime` for all ingested files. If metadata is unchanged and the status is `COMPLETED`, the scanner skips analysis, significantly reducing processing time for large, stable archives.
- **Handling Deleted Files:** Missing files are set to `DocumentStatus.NOT_PRESENT`, automatically purging them from the FTS index to ensure search results remain accurate.
- **Priority-Based Hydration:** Resuming scans prioritizes unprocessed files, followed by failed tasks, then retrying tasks to ensure steady progress. For evaluation, use `python src/scanner.py --judge` to track analysis quality.
