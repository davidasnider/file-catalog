...
- **SQLite Concurrency Management**: Uses semantic locking and FTS-specific semaphores to prevent "database is locked" errors during high-concurrency ingestion and indexing. Large batched reads (like fetching tasks for documents) use SQLite's `json_each()` function to bypass parameter limits of standard `IN()` clauses, with a chunked fallback for non-SQLite backends.
