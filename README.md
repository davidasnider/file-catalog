...
- **Optical Character Recognition (OCR)**: Automatically detects images and extracts text using Tesseract OCR (`pytesseract`). The `OCRConfidenceScorerPlugin` scores the quality of the extraction. The `VisionAnalyzerPlugin` runs independently to describe visual content.
