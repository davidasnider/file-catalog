...
- **Archive Extraction**: Archive extraction (Tar, Zip, 7z) must be hardened against path traversal. Use `tarfile.data_filter` for Tar files on Python 3.12+. For older versions or other formats like 7z, explicitly validate that both member paths and link targets (symlinks/hardlinks) resolve within the target destination directory.
