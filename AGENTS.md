...
- **Linting:** The project uses `ruff` for linting and formatting. Ensure pre-commit hooks are enabled.
  Code formatting and linting should be performed using `uv run ruff format <modified_files>` and
  `uv run ruff check <modified_files>`. Avoid running formatting on the entire codebase
  (e.g., `uv run ruff format .`). When formatting Markdown files with ruff, it will fail unless
  preview mode is enabled; use `uv run ruff format --preview <modified_files>`.
