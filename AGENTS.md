...
- **Archive Extraction**: Archive extraction (Tar, Zip, 7z) must be hardened against path traversal. For Tar files on Python 3.12+, use `extractall(dest, filter="data")`. For 7z archives using `py7zr`, `extractall` lacks a safe filter; use a manual loop iterating over `archive.list()`, validate that member paths (and symlink link targets) resolve within the destination, and only extract the validated members using `archive.extract(..., targets=[...])`. For ZIP files, validate member paths to prevent traversal (note: symlink link targets are not checked per-member).
