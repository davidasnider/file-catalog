...
# Invalidate failed tasks for retry
...
- **Archive Extraction**: Archive extraction (Tar, Zip, 7z) must be hardened against path traversal. For Tar files on Python 3.12+, use `extractall(..., filter="data")` to safely filter entries.
- **Email Attachments**: The `EmailParserPlugin` extracts email attachments to a dedicated `[file]_attachments/` directory located alongside the source email file.
